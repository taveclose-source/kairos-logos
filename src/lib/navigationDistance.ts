export const BOOK_START: Record<string, number> = {
...
export function getPageCount(
  ...
  toBook: string
): { pageCount: number; direction: 'forward' | 'back' } {
  ...
  const pageCount = Math.max(2, Math.round(2 + (distance * 18)))
  const direction: 'forward' | 'back' = toPos >= fromPos ? 'forward' : 'back'
  return { pageCount, direction }
}

export function getLastReadBook(): string | null {
