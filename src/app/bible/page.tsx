...
async function getBooks(): Promise<BookInfo[]> {
  ...
  // Get chapter summary via RPC (aggregated server-side, no row limit issue)
  const { data: chapterData } = await supabase.rpc('get_chapter_summary')
...
function BookCard({ book }: { book: BookInfo }) {
  ...
  return (
    <details className="group rounded-xl border border-gray-200">
      <summary className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-50 transition-colors list-none [&::-webkit-details-marker]:hidden">
