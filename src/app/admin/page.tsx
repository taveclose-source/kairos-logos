...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    sb.auth.getUser().then(({ data: { user } }) => {
      if (!user) { router.replace('/auth/signin?redirect=/admin'); return }
  ...
  // ── Users tab: filter, sort, paginate ──
  ...
  // Rough MRR estimate (can't distinguish monthly vs annual from tier alone)
    ...
    <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      <div className="flex border-b border-gray-200 mb-6 overflow-x-auto">
        {TABS.map((t) => (
          ...
          >
            {t.label}
            {t.id === 'missions' && missionUsers.filter((u) => u.missions_status === 'pending_approval').length > 0 && (
              <span className="ml-1.5 inline-block w-5 h-5 rounded-full bg-amber-500 text-white text-[10px] font-bold leading-5 text-center">
                {missionUsers.filter((u) => u.missions_status === 'pending_approval').length}
              </span>
            )}
            {tab === t.id && (
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <button
                    ...
                    disabled={page === 0}
                    className="px-3 py-1.5 rounded-lg text-sm border border-gray-200 hover:bg-gray-50 disabled:opacity-30 transition-colors"
                  >
                    &larr; Prev
                  </button>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-5">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    ${(sponsorships.reduce((sum, s) => sum + s.amount_cents, 0) / 100).toLocaleString()}
                  </p>
                </div>
